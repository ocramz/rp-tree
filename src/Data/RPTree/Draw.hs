...
draw :: (Show a, Show (v a)) => RPTree v a -> IO ()
draw = drawRPT . _rpTree

drawRPT :: (Show a, Show (v a)) => RPT v a -> IO ()
drawRPT = putStrLn . toStringRPT

toStringRPT :: (Show a, Show (v a)) => RPT v a -> String
toStringRPT = B.render . toBox

toBox :: (Show a, Show (v a)) => RPT v a -> B.Box
toBox = \case
  (Bin thr tl tr) ->
    txt (node thr) `stack` (toBox tl `byside` toBox tr)
  Tip x -> txt $ show x
  where
    node x = printf "%5.2f" (show x)
